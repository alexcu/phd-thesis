...
\todo{Introduce a broad statement of the problem. AI-first software is data-driven, but documentation and nature of services that provide these services make it look like they are deterministic} 
...
\begin{callout}
In this thesis, we advocate that the \textbf{(i)~integration}, \textbf{(ii)~developer comprehension}, \textbf{(iii)~documentation}, and \textbf{(iv)~quality attributes} of data-driven cloud services differ from the rule-driven nature of end-applications. As `intelligent' components contrast to traditional counterparts, and application developers need to take into account a greater appreciation of these factors.
\end{callout}
...
\section{Context: The `Spectrum' of Machine Learning}
\label{sec:introduction:context}

\input{mainmatter/introduction/tables/characteristics-of-cloud}

\todo{Unpack the `spectrum' a little, and make it clear that this thesis focuses on computer vision services.}
...
For the context of this thesis, we will refer to such services under broader term of \textit{\glspl{iws}}, and diagrammatically express their usage within \cref{fig:introduction:cloud-intelliegnce-service}.
...
\begin{callout}
There are different types of \glspl{iws} offered by service providers (such as optical character recognition, text-to-speech and speech-to-text transcription, object categorisation, facial analysis and recognition, natural language processing etc.). We scope the type of \glspl{iws} in this thesis to that of intelligent \gls{cv} analysis, which we refer to as \textbf{\glspl{cvs}}~\citepweb{GoogleCloud:Home,Azure:Home,AWS:Home,Pixlab:Home,IBM:Home,Cloudsight:Home,Clarifai:Home,DeepAI:Home,Imagaa:Home,Talkwaler:Home,Kairos:Home,Cognitec:Home,Affectiva:Home}. Evermore applications are using \glspl{cvs} as demonstrated by ubiquious examples: aiding the vision-impaired \citep{Reis:2018cp,daMotaSilveira:2017vp}, accounting  \citep{Marshall:2018uj}, data analytics \citep{Iyengar:2017fb}, and student education \citep{Dibia:2017iy}.
\end{callout}
