\begin{table}[hbt]
  ...
  \tablefit{\begin{tabular}{l|p{0.8\linewidth}}
    ...
    Provider A &
      ...
      \itshape
      ``Deprecated. Use score instead. The accuracy of the entity detection in an image. For example, for an image in which the `Eiffel Tower' entity is detected, this field represents the confidence that there is a tower in the query image. Range [0, 1].''~\upshape\citepweb{Google:DotNet}
      \bigskip

      \itshape
      ``The overall score of the result. Range [0, 1]''~\upshape\citepweb{Google:DotNet}
      \bigskip
    \\
    Provider B &
      \itshape
