...
%We then performed quantitative analyses on each response's labels, ensuring all labels were lowercased as case changed for \googleapi{} and \awsapi{} over the evaluation period. To derive at the consistency of responses for each image, we considered only the `top' $n$ labels per image for each \gls{api} and dataset. That is, for the same image $i$ over all images in dataset $D$ where $i \in D$ and over the three \glspl{api}, the top $n$ labels per image ($T_{i,n}$) of all labels per image $L_{i}$ (i.e., $T_{i,n} \subseteq L_{i}$) is that where the respective label's confidences are consistently the highest of all labels returned. 
