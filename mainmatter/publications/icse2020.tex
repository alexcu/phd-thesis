...
\paragraph{Defining a list of \glspl{iws}}
...
\paragraph{RQ1. How do developers mis-comprehend \glspl{iws} as presented within \gls{so} pain-points?}
%
...
\paragraph{RQ2. Are the distribution of issues similar to prior studies?}
...
\paragraph{Aligning \gls{so} taxonomies to Bloom's and \gls{solo} taxonomies}
...
\textit{(i)~Clarify use cases for \glspl{iws}:} Inspecting \gls{so} posts revealed that there is a level of confusion around the capabilities of different \glspl{iws}. This needs to be clarified in associated \glsac{api} documentation.
