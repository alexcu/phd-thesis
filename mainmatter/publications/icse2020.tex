...
{Interpreting Pain-Points in Computer Vision Services\pubfootnote{Cummaudo:2019vi}}
\label{ch:icse2020}
...
%%Problem: - Services are different to traditional services;  need a different type of documentation
...
%%Method: Look at it from previous distribution(s)
...
%%Findings: Technical domain matters to help us write better documentation
...
% Big picture context + Narrow picture focus
...
% Motivation to remove problem (i.e., gain in productivity or quality)
...
%Developers want to leverage the advances in AI and ML to provide value to end-users.
...
%Although some frameworks (e.g., Tensorflow) offer a middle-ground where developers can integrate pre-trained models or build on top of existing models without the need to setup a complete inference or training infrastructure. However, this is at the expense of the traits at the right-edge of the spectrum, where developers forgo full control over the system, thus not knowing how training is done or what training data is used to create the models.]{need to explain  this figure a bit more; AC - further explanation added}
...
\begin{enumerate}[label=\textbf{RQ\arabic*.},leftmargin=0.1125\linewidth]
  \item \textbf{\todo[RQ1. How do developers mis-comprehend intelligent services as presented within Stack Overflow pain-points?]{AC: Revised research question \comnum{3}}} While the AI community is well aware in the the nuances that empower intelligent services, such services are being released for application developers who may not be aware of their limitations or how they work. This is especially the case when machine intelligence is accessed via web-based APIs where such details are not fully exposed. 
  \item \textbf{Are the distribution of issues similar to prior studies?}  
  We compare how the distributions of previous studies' of posts about conventional, deterministic API services differ from those of intelligent services. \todo[By assessing the distribution of intelligent services' issues against existing surveys focusing on mobile and web development,]{AC: Reinforce that we are using these existing taxonomies for our purpose \comnum{9}} we identify whether a new taxonomy is needed specific to AI-based services, and if gaps specific to AI knowledge exist that need to be captured in these taxonomies.
\end{enumerate}


% \todo{AC: Our broader research aims to investigate the developer's experience when using intelligent services, thus our first steps are understanding the landscape of issues developers face before we address further issue (see \cref{icse2020:sec:limitations})}
% didn't use a custom classification taxonomy; justifed by using an existing one and see if it's sufficient --> future work explore further

\section{Background}
\label{icse2020:sec:related-work}

% Causal models -- motivation?
...
% Short paragraph about replication studies in SE - motivation?
...
\todo{AC: Removed text on EBSE \comnum{5}}.
% SE is not immune to the `replication crisis' of other fields (e.g., experimental psychology~\citep{Collaboration:2015uh}), and many have argued for the need to replicate studies as generalisation aids. Indeed, the idea is not new to SE as advocates as early as 1996 have argued for ``a better appreciation of the context of a piece of empirical work'' via the use of generalisation aids~\citep{Brooks:1996vv}. 
...
%\cref{icse2020:fig:method-process} shows an overview of the main activities we completed in our method mainly data extraction from SO, data filtering and data analysis.
% \begin{figure*}[t]
%   \centering
%   \includegraphics[width=\linewidth]{images/method-state-diagram.pdf}
%   \caption{
%     Overview of the processes undertaken in our method.
%   }
%   \label{icse2020:fig:method-process}
% \end{figure*}
...
Therefore, we applied the two existing taxonomies described in \cref{icse2020:sec:related-work} to each post; \todo[(i) a documentation-specific taxonomy that addresses issues directly resulting from documentation, and (ii) a generalised taxonomy that covers a broad range of SO issues in a well-defined SE area (specifically mobile app development).]{AC: Make it clearer that we're using a well-defined area \comnum{10}.}
...
\todo[\cref{icse2020:tab:taxonomies} is a transcription of \textit{our understanding} of the respective taxonomies; our intent is not to methodologically replicate \citet{Aghajani:2019bo} or \citeauthor{Beyer:2018fm}'s studies in the intelligent service domain, rather to acknowledge related work in the area of SO classification and reduce the need to synthesise a new taxonomy. We baselined all coding against \textit{our interpretation only}, and thus our classifications are therefore independent of previous findings as we baseline results via \cref{icse2020:tab:taxonomies}'s interpretation.]{AC: Discuss that we do not offer to mitigate bias introducing shared interpretation \comnum{9}}
...
% This can be formalised in the following for both Taxonomy A and Taxonomy B:
% \begin{align*}
%   H_0 & : h_{our\_study}(t) = h_{prior\_study}(t),   \qquad \text{for all $t \in[1,\tau]$}
%   \\
%   H_a & : h_{our\_study}(t) \ne h_{prior\_study}(t), \qquad \text{for some $t \in[1,\tau]$}
% \end{align*}

% Where \(\tau \) is the number of categories (7 for both Taxonomy A and Taxonomy B) and \textit{t} is a single category. 


\section{Findings}  
\label{icse2020:sec:findings}
%\input{tables/taxonomy-assignments}
...
%Refer to \cref{icse2020:tab:taxonomy-assignments} for a detailed breakdown of the number of posts allocated to only Taxonomy A ($A \setminus B$), B ($B \setminus A$), both ($A \cap B$), or neither ($(A \cup B)^{c}$).
...
% The percentage agreement and Light's Kappa statistic for Taxonomy A and B classifications are shown in \cref{icse2020:tab:reliability}. 
...
\todo[Despite all raters meeting to agree on a shared interpretation of the taxonomies (see \cref{icse2020:ssec:method:filtering:reliability}) our inter-rater measures aligned \textit{slightly} (0.148) for Taxonomy A and \textit{fairly} (0.295) for Taxonomy B. We report further in \cref{icse2020:sec:limitations}.]{AC: Report in-text reliability stats \comnum{7}}
...
% Aghajani (Taxonomy A) is hard to apply
...
% Why is it hard to apply? Because inference of problems ≠ direct links back to docs
...
% Developers want more information documented, but can't seem to find it
...
% Errors - "Help me I'm stuck" questions are most prevalent
...
% API Usage - Limitations between different CV vendors (what they can/cannot do isn't universal)
...
% Review - Understanding that these systems are trained with some data
...
% API change - evolution awareness not communicated/known
...
% ``\textit{Suddenly, the API started to respond differently to my requests. I sent the same picture to the API today, and I got a different response (from the past). I ensured nothing was changed in my code, so this is not the culprit.}''\solink{44740363} 


% We find most developers ask basic debugging questions reflecting a \textbf{basic understanding} of  computer vision and its key terminology and concepts. Some oversights include \textbf{the need to pre-process data} (i.e., these services are not `magic' and do not naturally work as human intelligence would presume), and the \textbf{limitations} between what different computer vision services offer is not well-understood (i.e., not all services provide the same functionality). \textbf{It is rare for developers to ask questions with ML techniques in mind}, such as the kind of \textbf{training data} that goes into these services and \textbf{how they evolve}. Moreover, very few developers seem to question or directly criticise the documentation of these services. 


% as 9.89\% of posts can be classified by \textit{both} taxonomies (See table \cref{icse2020:tab:taxonomy-assignments}.
...
%In this section, we offer an interpretation of our findings within the context of the intelligent services (specifically, computer vision services) in how they are being utilised by developers and note implications that arise.
...
\noindent
\textbf{\todo[RQ1. How do developers mis-comprehend intelligent services as presented within Stack Overflow pain-points?]{AC: Revised research question \comnum{3}.}}
% DISCUSSION of the reliability
...
% The distribution of posts under both Taxonomy A and B are contrasted between the results found in~\citet{Aghajani:2019bo} and~\citet{Beyer:2018fm}, respectively, within \cref{icse2020:fig:ab-compare}.
%~\citeauthor{Beyer:2018fm}'s taxonomy is generalised and thus presents a higher level of abstraction. Unsurprisingly, this taxonomy captured far more posts (\PctTaxBCategorised{}) than~\citeauthor{Aghajani:2019bo}'s documentation-specific taxonomy (\PctTaxACategorised{}). As discussed under RQ1's discussion, most developers simply do not ask questions that directly relate to intelligent service's documentation, and thus we cannot apply Taxonomy A to many posts. Developers mostly ask questions around technical aspects on SO than they do at a conceptual level.

% - CHANGES: Such API changes must be substantially documented. To address such issues,~\citet{Cummaudo:2019va} suggests that developers mitigate these threats by creating and testing against a representative sample of their data-sets.
% - CONCEPTS Recent work of~\citet{Cummaudo:2019va} reported that many such terminologies between vendors are different, and this shows that confusion begins to arise in developers as a result. 

% The error-related questions (B-3) from the Android study are significantly lower than that of our study (13.06\% vs. \PctTaxBErrors{}). One explanation for this discrepancy could be the relative complexity of the two domains. Intelligent services include distributed systems which is well known to be challenging~\cite{REF: } as well as being inherently probabilistic in nature. Another explanation could be the maturity of the two fields, the first version for Android was released in 2008 and the concerns for building a mobile app are well understood~\citep{Barnett2015, Barnett2015a}. In contrast to computer vision APIs that have typically been released in the last few years \footnote{https://cloud.google.com/vision/docs/release-notes\#may2017}\footnote{https://docs.aws.amazon.com/rekognition/latest/dg/document-history.html}. As a domain matures, we expect that developers gain a deeper appreciation of the technical domain concerns and as a consequence do not ask questions relating to errors. Further research to study the evolution of the issue types for Android and computer vision services is needed to evaluate this supposition.
...
% Knowledge (Remember) - recognising or remembering facts
...
% Comprehension (Understand) - understanding of facts and ideas by organising, comparing, interpreting etc.
...
% Application (Apply) - solving problems in new situations by applying acquired knowledge
...
% Analysis (Analyse) - examining and breaking information into component parts, determining how the parts relate to one another
...
% Evaluation (Evaluate) - presenting and defending opinions by making judgments about information
...
% Synthesise (Create) - building a structure or pattern from diverse elements
...
% Pre-structural - dev hasn't really understood the point 
...
% Uni-structural - only focuses on one relevant aspect
...
% Multi-structural - focuses on several relevant aspects but they are treated independently
...
% Relational -  aspects have become integrated into a coherent whole
...
% Extended abstract -  conceptualised at a higher level of abstraction and generalised to a new topic or area
...
\begin{table*}[tbh]
\centering
  \caption{Example Alignments of Stack Overflow posts to Bloom's and SOLO taxonomy.}
...
To understand our findings with the lenses of pedagogical aids, we aligned Taxonomies A and B to Bloom's and the SOLO taxonomies for a random sample of 50 issues described in \cref{icse2020:ssec:method:filtering:reliability}.  To do this, we reviewed all 50 of these SO posted questions and applied both the Bloom and SOLO taxonomies. The primary author assigned each of the 50 questions a level within the Bloom and SOLO taxonomies, removed out noise \todo[(i.e., false positive posts of no relevance to intelligent services)]{AC: Clarify `removed out noise' \comnum{11}} and unassigned dimensions from reliability agreement, and then compared the relevant dimensions of Taxonomy A and B dimensions (not sub-categories). The comparison of alignments of posts to the five SOLO dimensions and six Bloom dimensions are shown in \cref{icse2020:fig:alignment-of-blooms-solo}. 
We acknowledge that this is only an approximation of the current state of the developer's understanding of intelligent services. This early model will require further studies to perform a more thorough analysis, but we offer this interpretation for early discussion. %As the Bloom's and SOLO taxonomies are a learning hierarchy, we aligned lower orders of each hierarchy to questions posed within the random sample that pose low-order questions and vice-versa for the high-order questions.
...
% To derive implications from our findings we first contrast the initial Android API study with our own focusing on Intelligent Services. 
% Developers interacting with Android and Intelligent Service APIs face entirely different technical concerns when building an application. For example, Android developers must deal with sensors, intermittent connectivity and small screen sizes. These technical concerns impact the quality attributes of a system and are typically addressed through appropriate architectural patterns~\citep{Barnett:2018Kx}. For a mature domain such as Android app development the technical considerations are well understood~\citep{Barnett2015, Barnett2015a}. However, for less mature domains these fundamental concerns that influence software quality are not understood.   

% API Usage: Size of the APIs differ, Android ecosystem vs single function APIs 
% Errors: API Usage insufficient

% Understanding of the technical concerns: 

% Basic error-related questions

% Differences between Android APIs and Intelligent Services

\paragraph{Researchers}
\textit{(i)~Investigate the evolution of post classification:} Analysing how the distribution of the reported issues changes over time would be an important study. This study could answer questions like `Does the evolution of intelligent services follow the same pattern as previous software engineering trends such as mobile app or web development?' \todo[As with any new emerging field, it is key to analyse how developers perceive such issues with time; for instance, early issues with web or mobile app development matured as their respective domain matured, and we would expect similar results to occur in the intelligent services space. Future researchers could plan for a longitudinal study (such as a long-term survey or re-mining SO at a later date) to assess these trends and characteristics, and determine how and if the nature of the developer's experience with intelligent services and AI in general changes with time.]{AC: Way to mitigate this issue \comnum{18}.}
\textit{(ii)~Investigate the impact of technical challenges on API usage:} As discussed above, intelligent services have characteristics that may influence API usage patterns and should be investigated as a further avenue of research. \todo[Further mining of open source software repositories that make use of intelligent services could be assessed, thereby investigating if API patterns evolve with the rise of AI-based applications.]{AC: Way to mitigate this issue \comnum{18}.}

% Validating these taxonomies against a new domain has shown that the taxonomies are, therefore, still quite subjective in nature and presents the need for multi-faceted classification. As we discuss in \cref{icse2020:sec:limitations}, we only performed unary classification as per the techniques of previous manual studies (see \cref{icse2020:sec:related-work}). If the classifications are good and robust, then subjectivity would not be an issue. We have found, however, that the discrepancy in the interpretation the dimensions/sub-categories applied to SO posts is likely due to the variety of backgrounds and experience of our raters.  Thus, guidelines may need to be produced for researchers to apply the taxonomies in new areas.

\paragraph{Educators}
\label{icse2020:sssec:educators}

\textit{(i)~Education on high-level aspects of intelligent services:} As demonstrated in our analysis of their SO posts, many developers appear to be unaware of the higher-level concepts that exist within the AI and ML realm. This includes the need to pre- and post-process data, the data dependency and instability that exists in these services, and the specific algorithms that empower the underlying intelligence and hence their limitations and characteristics. However, most developers don't seem to complain about these factors due to the lack of documentation (i.e., via Taxonomy A). Rather, they are unaware that such information should be documentation and instead ask generalised and open questions (i.e., via Taxonomy B). Thus, documentation improvements alone may not be enough to solve these issues. This results in uncertainty during the preparation and operation (usage) of such services.  Such high-level conceptual information is currently largely missing in developer documentation for intelligent services. Furthermore, many of the background ML and AI algorithm information needed to understand and use intelligent systems in context are built within data science (not SE) communities. \todo[A possible roadmap to mitigate this issue would be the development of a developer `crash-course' in ML and AI to allow developers to gain an appreciation of its nuances and the inherent risk and implications that comes with using intelligent services. This could be taught at an undergraduate level to prepare the next generation of developers of a `programming 2.0' era---however, the key aspects and implications that are presented with AI would need to be curated for such an audience, and the best strategy to go about this we leave for the SE education domain.]{AC: Way to mitigate this issue \comnum{18}.}

\paragraph{Software Engineers}
\textit{(i)~Better understanding of intelligent API contextual usage:} Our results show that developers are still learning to use these APIs. We applied two learning perspectives to interpret our results. In applying the two pedagogical taxonomies to our findings, we see that most issues seem to fall into the pre-structural and knowledge-based categories; little is asked of higher level concepts and a majority of issues do not offer complex analysis from developers. This suggests that developers are struggling as they are unaware of the vocabulary needed to actually use such APIs, further reinforcing the need for API providers to write overview documentation (as noted in prior work \citep{Cummaudo:2019th}) and not just simple endpoint documentation. \todo[However, improved documentation isn't enough---as suggested by our discussion in \cref{icse2020:ssec:bloomsolo}, software engineers should explore further education to attain a greater appreciation of the nuances of ML when attempting to use these services.]{AC: Discuss how improved documentation isn't enough \comnum{20}}
...
\todo[The complication with this is pitching the documentation in a way that software developers, who are untrained in the nuances of AI and ML (see \cref{icse2020:sssec:educators}), would be able to digest it and apply it in-context to application development.]{AC: Complications around the issue \comnum{18}.}
...
\todo[A possible mitigation to this could be an interactive tutorial that helps developers fully understand the technical domain using a hands-on approach. For example, websites offer interactive Git tutorials\footnote{For example, \url{https://learngitbranching.js.org}} to help developers understand and explore technical domain matters in their own pace.]{AC: Way to mitigate this issue \comnum{18}.}
...
\todo[Such documentation could be provided with interactive tutorials.]{AC: Way to mitigate this issue \comnum{18}.}

% Technical domain matters; the distribution of AI-based issues differ from the distribution of Android issues, however distribution of documentation-related is largely the same. Paper from last year's ICPC (+ICSE). This said, few documentation issues.


% Loose approximation to approximate a model; we need a place to start...
% A more thorough work will be needed to do a better job
% Learning hierarchy; lowest order is vocaublary, as you go up the heirachy you need a lot of detailed understanding of it
% Developers are not able to learn these higher constructs, and hence it can be fixed through better documentation or education -- docs should focus on XYZ... because the whole category of errors are not uni-dimensional; the bulk of the errors are pre-structural, have no appreciation of the context
% If bulk of the issues are deep issues (i.e., high level) and the only place to fix this is better education (so the only option they have is better doucmentaion); documentation cannot be at the API endpoint levbel, take it at the level above (i.e., introductory stage)0
...
% Low IRR
\todo[As detailed in \cref{icse2020:ssec:method:filtering:reliability}, Taxonomy A and B present slight and fair agreement, respectively, when inter-rater reliability was applied. The nature of our disagreements largely fell due to the subjectivity in applying either taxonomies to posts. Despite all coders agreeing to the shared interpretation of both taxonomies, both taxonomies are subjective in their application, which was not reported by either \citeauthor{Aghajani:2019bo} or \citeauthor{Beyer:2018fm}'s works. In many cases, multi-classification seemed appropriate, however both taxonomies utilised single-label mapping which resulted in too much subjectivity, ultimately adversely affecting IRR analysis. This is also prevalent in other works (e.g. \citeauthor{Beyer:2018fm} plan for multi-label classification). Thus, a future mitigation strategy for similar work should explore multi-label classification to avoid this issue.]{AC: Address low IRR \comnum{1} \comnum{8}}
% Subjectivity of the labelling
...
% Only Alex did Bloom/SOLO classification
...
% Did not use multi-label classification
...
% We may have missed the selection of some posts
\subsection{External Validity} While every effort was made to select posts from SO relevant to computer vision services, there are some cases where we may have missed some posts. This is especially due to the case where some developers mis-reference certain intelligent services under different names (see \cref{icse2020:ssec:method:filtering:refining}). 
% Bloom's and SOLO may not applicable beyond intelligent APIs
Our SOLO and Bloom's taxonomy analysis has only been investigated through the lenses of intelligent services, and not in terms of conventional APIs (e.g., Andriod APIs). Therefore, we are not fully certain how these results found would compare to other types of APIs. %, however as far as we are aware, this is the first attempt at aligning SO posts with either the Bloom's or SOLO taxonomies, and we therefore leave such investigations for future work.
% Why use existing taxonomies instead of developing own? Why topic modelling not used?
Two \textit{existing} SO classification taxonomies were used rather than developing our own. We wanted to see if previous SO taxonomies could be applied to intelligent services before developing a new, specific taxonomy,\todo[and these taxonomies were applied based on our interpretation  (see \cref{icse2020:ssec:method:filtering:classification}) and may not necessarily reflect the interpretation of the original authors of these taxonomies]{AC: Discuss that we do not offer to mitigate bias introducing shared interpretation \comnum{9}}. Moreover, automated techniques such as topic modelling were not utilised as we found these produce descriptive classifications only (see \cref{icse2020:sec:related-work}). Hence, manual analysis was performed by humans to ensure categories could be aligned back to causal factors.
% We only chose English Speaking services to analyse
...
% Cmputer vision may not generalise
\todo[Use of computer vision in this study is an illustrative example to focus on one area of the intelligent services spectrum. While our narrow scope helps us obtain more concrete findings, we suggest that wider issues exist in other intelligent service domains may affect the generalisability of this study, and suggest future work be explored in this space.]{AC: Add threat about generalisability \comnum{21}}

% False positives identified
\subsection{Construct Validity} Some questions extracted from SO produced false positives, as mentioned in \cref{icse2020:ssec:method:filtering:refining,icse2020:ssec:method:filtering:automated-manual-filtering,icse2020:sec:findings}. However, all non-relevant posts were marked as noise for our study, and thus did not affect our findings.
% Stack Overflow questions are asked when there is a basic answer already
...
Intelligent services, such as computer vision services, offer powerful capabilities that can be added into the developer's toolkit via simple RESTful APIs. However, certain technical nuances of computer vision become abstracted away. We note that this abstraction comes at the expense of a full appreciation of the technical domain, context and proper usage of these systems. We applied two recent existing SO classification taxonomies (from 2018 and 2019) to see if existing taxonomies are able to fully categorise the types of complaints developers have. Intelligent services have a diverging distribution of the types of issues developers ask when compared to more mature domains (i.e., mobile app development and web development). Developers are more likely to complain about shallower, simple debugging issues without a distinct understanding of the AI algorithms that actually empower the APIs they use. Moreover, developers are more likely to complain about the completeness and correctness of existing intelligent service documentation, thereby suggesting that the documentation approach for these services should be reconsidered. Greater attention to education in the use of AI-powered APIs and their limitations is needed, \todo[and our discussion offered in \cref{icse2020:ssec:bloomsolo} motivates future work in resolving these issues in the SE education space.]{AC: Reinforce that Bloom/SOLO is a discussion item \comnum{12}}.


% - What did we find in the context of replication?
% --> Categorisation is Lanza is not enough -- misses cognitive priors;
% --> Beyer is high level picks up all but misses detail
% --> Nature of the cognitive priors specific CV
% --> issues/cog priors are spread out
% - The method used in the two papers 
% -- Higher level of abstraction, unsurprisingly, captured more results. Not as casual.
% -- Lower level of abstraction (docs), did not -- actionable is missing. Too narrow; no one is really asking it
% - As a critique; these taxonomies are still not good enough
% -- People don't really ask at a low level; then general ask at a high level
% -- Implications; if your docs are at a slightly high level, you can generally get away with it
% - We agree with these papers


% List of contributions???
%Contribution 1:  Union of (1) and (2); beyer is better and ***different***
%Contribution 2:  Correction for (3) -- education; causal ...
%Contribution 3:  .. to CV (investigate against a learning taxonomy Blooms Taxonomy / SOLO taxonomy) -- link .
%Contribution 4:  Replication of previous work (extraction) to see if the extraction method is robust; it is - and we have built upon it
%
%Contribution 1:  Intelligent services has a new category of errors (different distribution of what ppl are complaining) -- context + domain matters; catgeory of errors are different as aAI
%Engineers are learning, hence we looked at some learning classification frameworks, take a small sample of these issues and isolate them... future work will do a more comprehensive thing at it
%C2: Most of them are coming at the lower end of the Bloom's taxonomy........ clearly articulate that the field is not new (Andriod, questions at higher end of taxonomy)